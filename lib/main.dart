...
class _ConverterScreenState extends State<ConverterScreen> {
  ...
  double _pitch = 0.0;   
  double _roll = -90.0;  
  String _status = 'Настройте ювелирные углы, FOV и скопируйте команду';
...
for file in *.insp; do
  ...
  base="\${file%.*}"
  echo "Обработка: \$file"
  ffmpeg -i "\$file" -filter_complex "[v:0]crop=iw/2:ih:0:0,v360=input=fisheye:output=hequirect:h_fov=$fov:v_fov=$fov:yaw=0:pitch=0:roll=0[left_eye]; [v:0]crop=iw/2:ih:iw/2:0,v360=input=fisheye:output=hequirect:h_fov=$fov:v_fov=$fov:yaw=${_yaw.toStringAsFixed(2)}:pitch=${_pitch.toStringAsFixed(2)}:roll=${_roll.round()}[right_eye]; [left_eye][right_eye]hstack=inputs=2[sbs]" -map "[sbs]" -y "$_outputDir/\${base}_sbs_180.png"
done
      ...
      body: Padding(
        ...
        child: Column(
          children: [
            ...
            // Yaw: от -2 до 2, шаг 0.01 (всего 400 делений), вывод 2 знаков после запятой
            _buildSlider(
              label: 'Yaw (Рыскание)', 
              val: _yaw, min: -2.0, max: 2.0, divisions: 400, fractionDigits: 2,
              cb: (v) => setState(() => _yaw = v),
            ),
            
            // Pitch: от -2 до 2, шаг 0.01 (всего 400 делений), вывод 2 знаков после запятой
            _buildSlider(
              label: 'Pitch (Тангаж)', 
              val: _pitch, min: -2.0, max: 2.0, divisions: 400, fractionDigits: 2,
